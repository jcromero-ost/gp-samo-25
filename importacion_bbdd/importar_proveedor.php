...
$reader = new DBFReader('C:\\SAMO\\ClasGes6SP26\\DATOS\\proveedo.dbf');
...
if (in_array('CLAPRO', $columns)) {
    $fieldDefs[] = "PRIMARY KEY (`CLAPRO`)";
}

// Crear tabla si no existe
$tableSQL = "CREATE TABLE IF NOT EXISTS cg_proveedores (
" . implode(",\n", $fieldDefs) . "
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;";
$pdo->exec($tableSQL);

// Obtener columnas válidas existentes
$existingCols = $pdo->query("DESCRIBE cg_proveedores")->fetchAll(PDO::FETCH_COLUMN);
...
for ($i = 0; $i < $totalRecords; $i += $batchSize) {
    ...
    if (!empty($insertValues)) {
        ...
        $sql = "INSERT INTO cg_proveedores (" . implode(',', $insertCols) . ")
...
$ids_dbf = array_column($reader->getRecords(0, $reader->getRecordCount()), 'CLAPRO');
$stmt = $pdo->query("SELECT CLAPRO FROM cg_proveedores");
...
if (!empty($ids_a_eliminar)) {
    ...
    $del = $pdo->prepare("DELETE FROM cg_proveedores WHERE CLAPRO IN ($in)");
