...
if (in_array('CLAART', $columns)) {
    $fieldDefs[] = "PRIMARY KEY (`CLAART`)";
}

// Crear tabla si no existe
$tableSQL = "CREATE TABLE IF NOT EXISTS cg_articulos (
" . implode(",\n", $fieldDefs) . "
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;";
$pdo->exec($tableSQL);

// Obtener columnas reales existentes
$existingCols = $pdo->query("DESCRIBE cg_articulos")->fetchAll(PDO::FETCH_COLUMN);
...
for ($i = 0; $i < $totalRecords; $i += $batchSize) {
    ...
    if (!empty($insertValues)) {
        ...
        $sql = "INSERT INTO cg_articulos (" . implode(',', $insertCols) . ")
...
$ids_dbf = array_column($reader->getRecords(0, $reader->getRecordCount()), 'CLAART');
$stmt = $pdo->query("SELECT CLAART FROM cg_articulos");
...
if (!empty($ids_a_eliminar)) {
    ...
    $del = $pdo->prepare("DELETE FROM cg_articulos WHERE CLAART IN ($in)");
