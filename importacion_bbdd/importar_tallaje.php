...
$reader = new DBFReader('C:\\SAMO\\ClasGes6SP26\\DATOS\\tallajes.dbf');
...
if (in_array('CLATAL', $columns)) {
    $fieldDefs[] = "PRIMARY KEY (`CLATAL`)";
}

// Crear tabla si no existe
$tableSQL = "CREATE TABLE IF NOT EXISTS cg_tallaje (
" . implode(",\n", $fieldDefs) . "
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;";
$pdo->exec($tableSQL);

// Obtener columnas reales existentes
$existingCols = $pdo->query("DESCRIBE cg_tallaje")->fetchAll(PDO::FETCH_COLUMN);
...
for ($i = 0; $i < $totalRecords; $i += $batchSize) {
    ...
    if (!empty($insertValues)) {
        ...
        $sql = "INSERT INTO cg_tallaje (" . implode(',', $insertCols) . ")
...
$ids_dbf = array_column($reader->getRecords(0, $reader->getRecordCount()), 'CLATAL');
$stmt = $pdo->query("SELECT CLATAL FROM cg_tallaje");
...
if (!empty($ids_a_eliminar)) {
    ...
    $del = $pdo->prepare("DELETE FROM cg_tallaje WHERE CLATAL IN ($in)");
