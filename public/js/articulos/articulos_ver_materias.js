document.addEventListener('DOMContentLoaded', () => {
  ...
  window.cargarMaterias = function(codpadre, container, page = 1, limit = 5) {
    ...
    // Realizar petición POST para obtener las líneas del pedido según CODPADRE, offset y limit
    fetch('./articulos_ver_materias', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      // Enviar los parámetros codificados en el cuerpo de la petición
      body: `CODPADRE=${encodeURIComponent(codpadre)}&offset=${offset}&limit=${limit}`
    })
    ...
    .then(response => {
      ...
      if (!data || data.length === 0) {
        container.innerHTML = '<p>No hay materias primas para este articulo</p>';
        return;
      }
      ...
      if (page > 1) {
        html += `<button class="btn btn-sm btn-outline-primary prev-materias">Anterior</button>`;
      } else {
      ...
      if (page < totalPages) {
        html += `<button class="btn btn-sm btn-outline-primary next-materias">Siguiente</button>`;
      } else {
        html += `<span></span>`; // Espacio vacío para mantener el diseño
      }
      ...
      if (prevBtn) {
        prevBtn.addEventListener('click', () => {
          window.cargarMaterias(codpadre, container, page - 1, limit);
        });
      }
    ...
    .catch(() => {
      ...
      container.innerHTML = `<p class="text-danger">Error al cargar las materias primas.</p>`;
    });
  };
});
